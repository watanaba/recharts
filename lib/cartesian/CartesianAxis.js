...
var CartesianAxis = /*#__PURE__*/function (_Component) {
  ...
  _createClass(CartesianAxis, [{
    ...
    key: "timeTickFormatter",
    value: function timeTickFormatter(value, month) {
      var shortMonthStr = ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec'];
      var t = new Date(value * 1000);

      if (month) {
        return "".concat(shortMonthStr[t.getUTCMonth()], " '").concat(t.getUTCFullYear() % 100);
      }

      return "".concat(shortMonthStr[t.getUTCMonth()], " ").concat(t.getUTCDate());
    }
  }, {
    key: "formatTimeTicks",
    value: function formatTimeTicks(ticks, month) {
    ...
    key: "shouldShowForWeek",
    value: function shouldShowForWeek(value, interval) {
      if (interval === 1) return true;
      var baseSunday = 3 * 24 * 60 * 60; // 1970年1月4日は日曜日（UNIXエポックからの4日目）

      var weeksSinceBase = Math.floor((value - baseSunday) / (7 * 24 * 60 * 60));
    ...
    key: "shouldShowForMonth",
    value: function shouldShowForMonth(value, interval) {
      if (interval === 1) return true;
    ...
    key: "getTimeTicks",
    value: function getTimeTicks(ticks) {
      var result = (ticks || []).slice();
